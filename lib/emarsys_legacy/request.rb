module EmarsysLegacy
  ...
  class Request
    ...
    def send_request
      ...
      when :post
        RestClient.post(emarsys_uri, converted_params.to_json, :content_type => :json, :x_wsse => client.x_wsse_string) do |response, request, result, &block|
          EmarsysLegacy::Response.new(response).result
        end
      when :put
    ...
    def client
      EmarsysLegacy::Client.new
    end

    def emarsys_uri
      [EmarsysLegacy.api_endpoint, @path].join('/')
    end

    def converted_params
      EmarsysLegacy::ParamsConverter.new(params).convert_to_ids
    end
  end
