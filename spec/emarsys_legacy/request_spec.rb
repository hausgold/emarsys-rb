...
describe EmarsysLegacy::Request do
  let(:client) { EmarsysLegacy::Client.new }
  let(:request) { EmarsysLegacy::Request.new('get', 'some-path', {:a => 1}) }
  ...
  describe '#client' do
    it "provides a simple client accessor method" do
      expect(request.client).to be_a(EmarsysLegacy::Client)
    end
  end
